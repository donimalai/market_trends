...
st.markdown(
    ...
    "**Macro overlay uses two lines on two independent axes.** ASX 200 close reads off "
    "the left axis (points); the RBA cash rate is its own line, in its native units (%), "
    "reading off a separate right-hand axis — each series is readable without mentally "
    "rescaling one against the other. The cash rate is drawn as a **step line**, not a "
    "straight interpolation between points, because the rate is genuinely constant "
    "between RBA decisions and jumps discretely on a change — a straight line between "
    "two rate points would visually imply a gradual move that never happened. The "
    "correlation sentence below the chart is computed from the full `cash_rate` series "
    "independently of the chart itself.\n\n"
